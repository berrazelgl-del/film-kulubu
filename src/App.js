...
const styles = {
  ...
  card: {
    ...
    padding: 15,
    borderRadius: 20,
    boxShadow: "0 8px 20px rgba(155, 89, 182, 0.2)",
    transition: "all 0.3s",
  }
};
...
function Filters({ genres, filters, onChange, onReset }) {
  return (
    <div style={{ 
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: "600", color: "#6c3483" }}>
        🎬 Tür:
        <select
          value={filters.genre}
      ...
      <button onClick={onReset} style={{ ...styles.button, background: "linear-gradient(135deg, #e8daef 0%, #d7bde2 100%)", color: "#6c3483" }}>
        🔄 Sıfırla
      </button>
    </div>
  );
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => {
        ...
        e.currentTarget.style.boxShadow = "0 8px 20px rgba(155, 89, 182, 0.2)";
      }}
    >
      {img ? (
        <img src={img} alt={show.name} style={{ width: "100%", borderRadius: 15, border: "3px solid #d7bde2" }} />
      ) : (
        <div style={{ 
          height: 300, 
      ...
      <h3 style={{ margin: 0, color: "#6c3483", fontSize: 18 }}>{show.name}</h3>
      <div style={{ fontSize: 13, color: "#9b59b6", fontWeight: "500" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        {isInWatchlist ? (
          <button onClick={() => onRemove(show.id)} style={{ ...styles.button, background: "linear-gradient(135deg, #e8daef 0%, #d7bde2 100%)", color: "#6c3483", flex: 1, fontSize: 13 }}>
            ❌ Kaldır
          </button>
        ) : (
...
function TVList({ items, watchlistIds, onAdd, onRemove, onShowDetail }) {
  if (!items.length) return <div style={{ textAlign: "center", color: "#6c3483", fontSize: 18, padding: 40 }}>💜 Sonuç bulunamadı.</div>;
  return (
...
function WatchlistPanel({ items, onRemove, onClear, onShowDetail }) {
  return (
    <div style={{ 
      ...
      <h3 style={{ color: "#6c3483", display: "flex", alignItems: "center", gap: 8 }}>
        💜 Gösterime Girecekler ({items.length})
      </h3>
      {items.length === 0 && <div style={{ color: "#9b59b6", fontSize: 14 }}>Henüz eklenmiş dizi yok.</div>}
      {items.map((s) => (
        <div key={s.id} style={{ 
          marginBottom: 12, 
          padding: 12, 
          background: "linear-gradient(135deg, #fbf9fc 0%, #f4ecf7 100%)",
          border: "2px solid #d7bde2",
          borderRadius: 15,
        }}>
          <strong style={{ color: "#6c3483" }}>{s.name}</strong>
          <div style={{ fontSize: 12, color: "#9b59b6", marginTop: 4 }}>{s.language}</div>
          <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
            <button onClick={() => onShowDetail(s)} style={{ ...styles.button, fontSize: 12, padding: "6px 12px" }}>
              📖
            </button>
            <button onClick={() => onRemove(s.id)} style={{ ...styles.button, fontSize: 12, padding: "6px 12px", background: "linear-gradient(135deg, #e8daef 0%, #d7bde2 100%)", color: "#6c3483" }}>
              ❌
            </button>
          </div>
        </div>
...
function Pagination({ totalItems, pageSize, page, onSetPage }) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      ...
      <div style={{ fontWeight: "600", color: "#6c3483", padding: "0 10px" }}>
...
function ShowDetail({ show, onClose }) {
  ...
  return (
    ...
    >
      <div
        style={{
          background: "linear-gradient(135deg, #ffffff 0%, #fbf9fc 100%)",
          borderRadius: 25,
          padding: 30,
          maxWidth: 800,
          width: "100%",
      ...
      >
        <h2 style={{ color: "#6c3483", marginBottom: 15 }}>💜 {show.name}</h2>
        <div dangerouslySetInnerHTML={{ __html: show.summary || "" }} style={{ color: "#666", lineHeight: 1.6 }} />
        <h4 style={{ color: "#9b59b6", marginTop: 20 }}>📺 Bölümler</h4>
        {loading && <div style={{ color: "#9b59b6" }}>Yükleniyor...</div>}
        {error && <div style={{ color: "#6c3483" }}>Hata: {error}</div>}
        {!loading && !error && (
          <ul style={{ maxHeight: 300, overflow: "auto" }}>
            {episodes.map((ep) => (
              <li key={ep.id} style={{ color: "#666", marginBottom: 8 }}>
                <strong style={{ color: "#9b59b6" }}>{ep.season}x{ep.number}</strong> — {ep.name}
              </li>
            ))}
          </ul>
        )}
...
function Footer() {
  return (
    <footer style={{ 
      ...
      <small>💜 Hazırlayan: Şadiye Berra Özelgül (Kampüs Film Kulübü Ödevi)-2025 💜</small>
    </footer>
  );
}
  ...
  return (
    <div style={{ display: "flex", gap: 20 }}>
      <div style={{ flex: 1 }}>
        ...
        {loading && <div style={{ textAlign: "center", color: "#6c3483", fontSize: 18 }}>💜 Yükleniyor...</div>}
        {error && (
          <div style={{ textAlign: "center", color: "#6c3483" }}>
            Hata oluştu: {error} <button onClick={() => fetchByQuery(dispatch, query)} style={styles.button}>Tekrar dene</button>
          </div>
        )}
        {!loading && !error && filtered.length === 0 && <div style={{ textAlign: "center", color: "#6c3483", fontSize: 18 }}>💜 Sonuç bulunamadı.</div>}
        {!loading && !error && filtered.length > 0 && (
          <>
            ...
            <div style={{ 
              ...
              />
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: "600", color: "#6c3483" }}>
                📄 Sayfa boyutu:
                <select
                  value={pageSize}
...
export default function App() {
  ...
  return (
    <div style={{ 
      minHeight: "100vh",
      background: "linear-gradient(135deg, #d7bde2 0%, #c39bd3 25%, #af7ac5 50%, #c39bd3 75%, #d7bde2 100%)",
      padding: 30,
      ...
      <header style={{ 
        ...
        <p style={{ color: "#9b59b6", margin: "10px 0 0 0", fontSize: 16, fontWeight: "600" }}>
